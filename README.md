...
All appointments live in PostgreSQL, regardless of where they were created. Connecting imports the current and following two calendar years to cover the allowed start and duration window; after that, [Google sync tokens](https://developers.google.com/workspace/calendar/api/guides/sync) bring in new, changed or deleted appointments. Browsing dates reads the database without contacting Google. Recurring and all-day appointments are supported.
...
| Incoming Google changes | One sync token per calendar and one row per event. Changes and the next token commit together after all pages succeed. Failed syncs keep existing data; overlapping jobs share one request. Expired tokens, an older import horizon or year rollover trigger a full refresh. |
